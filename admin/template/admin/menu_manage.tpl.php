...
<td><a href="?mod=admin&file=menu&action=manage&parentid=<?=$menu['menuid']?>&parentname=<?=urlencode($menu['name'])?>"><?=$menu['name']?></a></td>
...
<?php if($menu['isfolder']){ ?><a href="?a=admin.menu.manage&parentid=<?=$menu['menuid']?>&parentname=<?=urlencode($menu['name'])?>">子菜单</a> <?php }else{ ?><font color="#cccccc">子菜单</font><?php } ?> |
<a href="?a=admin.menu.edit&menuid=<?=$menu['menuid']?>">修改</a> |
