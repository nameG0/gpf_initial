...
?>
<body onLoad="$('#setting').load('?mod=<?=$mod?>&file=<?=$file?>&action=setting_edit&modelid=<?=$modelid?>&fieldid=<?=$fieldid?>&formtype=<?=$formtype?>');">
<?=$menu?>
...
?>
	...
	<tr>
      ...
      <td><input type="radio" name="info[isfulltext]" value="1" <?=($info['isfulltext'] ? 'checked' : '')?> /> 是 <input type="radio" name="info[isfulltext]" value="0"  <?=($isfulltext ? '' : 'checked')?> /> 否</td>
    </tr>
	<tr>
