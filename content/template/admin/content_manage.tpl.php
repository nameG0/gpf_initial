...
-->
<?=output::style($info['title'], $info['style'])?><!--</a>--> <?=$info['thumb'] ? '<font color="red">图</font>' : ''?>&nbsp;<?=$info['posids']?'<font color="green">荐</font>': ''?>&nbsp;<?=$info['typeid']?'<font color="blue">类</font>': ''?></td>
...
-->
<a href="<?=admin_url("..edit.&contentid={$info['contentid']}&catid={$info['catid']}")?>">修改</a> |
<a href="<?=admin_url("..delete.&contentid={$info['contentid']}&catid={$info['catid']}")?>">删除</a>
<!--
...
<span style="width:60px"><a href="###" onclick="javascript:$('input[type=checkbox]').attr('checked', true)">全选</a>/<a href="###" onclick="javascript:$('input[type=checkbox]').attr('checked', false)">取消</a></span>
		...
		-->
		<input type="button" name="delete" value=" 删除 " onclick="myform.action='<?=admin_url("..delete..catid")?>';myform.submit();"> 
		<!--
