...
class model
{
	...
	function add($model)
	{
		if(!is_array($model) || empty($model['name']) || empty($model['tablename']))
			{
			log::add("提交数据不完整", log::INPUT, __FILE__, __LINE__, __CLASS__.'->'.__FUNCTION__);
			return false;
			}
		if($this->db->table_status(DB_PRE.'c_'.$model['tablename']))
			{
			log::add("数据表已存在", log::INPUT, __FILE__, __LINE__, __CLASS__.'->'.__FUNCTION__);
			return false;
			}
		...
		$sql = file_get_contents(CONTENT_ROOT . 'install/model.sql');
