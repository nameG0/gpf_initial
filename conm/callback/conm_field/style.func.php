 ...
 * @filesource
 */
function cm_ft_conm__style_sql($set, $rdb_type = NULL)
{//{{{
	//todo 目前 $rdb_type 参数无效。
	return "CHAR( 5 ) NOT NULL DEFAULT ''";
}//}}}
...
function cm_ft_conm__style_form($field, $value, $fieldinfo)
{//{{{
	global $catid;
	extract($fieldinfo);
...
function cm_ft_conm__style_input()
{//{{{
	
}//}}}

function cm_ft_conm__style_update()
{//{{{
	
}//}}}

function cm_ft_conm__style_search_form($field, $value, $fieldinfo)
{//{{{
	return form::text($field, $field, $value, 'text', 20);
}//}}}

function cm_ft_conm__style_search($field, $value)
{//{{{
	return $value === '' ? '' : " `$field` LIKE '%$value%' ";
}//}}}

function cm_ft_conm__style_output($field, $value)
{//{{{
