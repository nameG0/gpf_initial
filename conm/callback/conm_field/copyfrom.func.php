 ...
 * @filesource
 */
function cm_ft_conm__copyfrom_sql($set, $rdb_type = NULL)
{//{{{
...
function cm_ft_conm__copyfrom_FString($set)
{//{{{
	if(!$set['maxlength']) $set['maxlength'] = 255;
	$maxlength = min($set['maxlength'], 255);
...
function cm_ft_conm__copyfrom_setting($setting)
{//{{{
	?>
...
function cm_ft_conm__copyfrom_form($field, $value, $fieldinfo)
{//{{{
	global $catid;
	extract($fieldinfo);
...
function cm_ft_conm__copyfrom_input()
{//{{{
	
}//}}}

function cm_ft_conm__copyfrom_update()
{//{{{
	
}//}}}

function cm_ft_conm__copyfrom_search_form($field, $value, $fieldinfo)
{//{{{
	return form::text($field, $field, $value, 'text', 20);
}//}}}

function cm_ft_conm__copyfrom_search($field, $value)
{//{{{
	return $value === '' ? '' : " `$field` LIKE '%$value%' ";
}//}}}

function cm_ft_conm__copyfrom_output($field, $value)
{//{{{
