 ...
 * @filesource
 */
function cm_ft_conm__groupid_sql($set, $rdb_type = NULL)
{//{{{
	//todo 目前 $rdb_type 参数无效。
	return "TINYINT(1) UNSIGNED NOT NULL DEFAULT 0";
}//}}}
function cm_ft_conm__groupid_FString($set)
{//{{{
	return "tinyint(1) unsigned|NO|0|";
}//}}}

function cm_ft_conm__groupid_setting($setting)
{//{{{
	?>
...
function cm_ft_conm__groupid_form($field, $value, $fieldinfo)
{//{{{
	global $catid;
	extract($fieldinfo);
...
function cm_ft_conm__groupid_input()
{//{{{
	
}//}}}

function cm_ft_conm__groupid_update()
{//{{{
	
}//}}}

function cm_ft_conm__groupid_search_form($field, $value, $fieldinfo)
{//{{{
	return form::text($field, $field, $value, 'text', 20);
}//}}}

function cm_ft_conm__groupid_search($field, $value)
{//{{{
	return $value === '' ? '' : " `$field` LIKE '%$value%' ";
}//}}}

function cm_ft_conm__groupid_output($field, $value)
{//{{{
