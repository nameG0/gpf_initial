...
function paging_style_default($pageCount, $current, $urlrule)
{
	...
	if ($pageCount > 1)
		...
		if ($current < $pageCount)
			...
			{
			$ret .= " <u><b title=\"{$pageCount}\">{$pageCount}</b></u><a href=\"" . paging_url($urlrule, $current) . "\" title=\"{$PAGING_LANG['next']}\" >{$PAGING_LANG['next_button']}</a> ";
			}
