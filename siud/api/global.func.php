...
{//{{{
	...
	//ggzhu@2012-12-19 当 $pagesize=0 时不进行分页。
	if (false === $pagesize)
		{
		//此时 $page 表示 LIMIT
		if ($page)
	...
	$other['style'] = $other['style'] ? $other['style'] : 'default';
	if ($other['sql_count'])
		{
		...
		if (false === stripos($sql_count, 'DISTINCT'))
			...
			{
			//SELECT DISTINCT userid >> SELECT COUNT(DISTINCT userid)
	...
	$result = $db->select($sql);
	$pages = paging($count, $pagesize, $page, $url, $other['style']);
	return array($result, $pages, $count);
}//}}}

/**
 * 缓存 SELECT COUNT(*) 的结果，设计用于缓存前台列表页的记录总数量。
 * @param int $CACHE_COUNT_TTL 缓存有效期，秒
 */
function cache_count($sql, $CACHE_COUNT_TTL = CACHE_COUNT_TTL)
{//{{{
	global $TEMP;
	$db = rdbApi::obj();

	gpf::log($CACHE_COUNT_TTL, gpf::INFO, __FILE__, __LINE__, __FUNCTION__);
	$id = md5($sql);
	$key = "cache_count_{$id}";
	if(!isset($TEMP[$key]))
	{
